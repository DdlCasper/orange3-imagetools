...
class OWRowSelector(OWWidget):
    ...
    def parse_indices(self, text, max_len): #This function transforms the text in a logic that the software can understand.
        ...
        if not text.strip(): #Removes whitespaces
            return []
        indices = set()
        parts = text.split(',') #It splits each part of the string using "," as a separator
        
        for part in parts:
            part = part.strip() #Removes whitespaces again in case someone writes 5 - 7 instead of 5-7
            if not part:
                continue
            ...
            if '-' in part: #If part of the string has "-"...
                ...
                try: #If the part is just the number, it converts it into an integer and appends it to the list of indeces
                    val = int(part)
                    if 1 <= val <= max_len:
                        indices.add(val - 1)
                except ValueError:
